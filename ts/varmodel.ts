...
type SortFunction = (a: Location, b: Location) => number;
...
export function normalizeLocation(location: Location, axes: Axis[]): Location {
	var out: Location = {};
...
export function supportScalar(location: Location, support: Support): number {
	let scalar = 1;
...
export class VariationModel {
	...
	constructor(locations: Location[], axisOrder: string[]) {
		this.axisOrder = [];
		...
		for (var loc of this.origLocations) {
			loc2.push(
				Object.keys(loc)
					...
					.reduce((obj: Location, key: string) => {
						obj[key] = loc[key];
						return obj;
					}, {})
			);
	...
	getMasterLocationsSortKeyFunc(
		...
		var func = function (a: Location, b: Location): number {
	...
	_locationsToRegions(): Support[] {
		let locations: Location[] = this.locations;
		let minV: Location = {};
		let maxV: Location = {};
	...
	getScalars(loc: Location): number[] {
	...
	interpolateFromDeltas(loc: Location, deltas: number[]): number | null {
		let scalars = this.getScalars(loc);
		return this.interpolateFromDeltasAndScalars(deltas, scalars);
	}

	interpolateFromMasters(loc: Location, masterValues: number[]) {
